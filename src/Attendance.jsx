...
export default function Attendance() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const sRes = await fetch(getReadUrl("학생 신청"));
      if (!sRes.ok) throw new Error("시트를 불러올 수 없어요.");
      const sJson = await sRes.json();
      const values = sJson.values || [];
      setStudentData(values);
      // 장소별 현재 인원 계산 (D열=창의1인원, G열=창의2인원 — 시트에서 직접 읽음)
      const counts1 = {};
      const counts2 = {};
      ...
      if (rowIndex !== -1) {
        ...
        setForm({
          loc1: values[rowIndex][2] || "",
          loc2: values[rowIndex][5] || "",
        });
      }

      const oRes = await fetch(getReadUrl("외출 신청"));
  ...
  const handleSubmit = async () => {
    console.log("신청 시작!", { form, myRow }); // 이 줄 추가
    if (!form.loc1 && !form.loc2) {
      setSubmitMsg("장소를 하나 이상 선택해줘!");
      return;
    }
    if (!myRow) {
      setSubmitMsg("내 학번을 시트에서 찾을 수 없어요.");
      return;
    }
    ...
    const token = sessionStorage.getItem("sheets_token");
    const sheetRow = myRow.index + 1; // 시트는 1-indexed
        ...
        requests.push(
          fetch(
            `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(SHEET_NAME)}!C${sheetRow}?valueInputOption=USER_ENTERED&access_token=${token}`,
            {
              ...
              body: JSON.stringify({ values: [[form.loc1]] }),
            }
          )
        );
      }
        ...
        requests.push(
          fetch(
            `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(SHEET_NAME)}!F${sheetRow}?valueInputOption=USER_ENTERED&access_token=${token}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ values: [[form.loc2]] }),
            }
          )
        );
      }
  ...
  return (
    <div className="attendance-wrap">
      ...
      <div className="tab-bar">
        ...
        <button className={`tab-btn ${tab === "outing" ? "active" : ""}`} onClick={() => setTab("outing")}>
          🚪 외출 신청
        </button>
      </div>
      ...
      {!loading && !error && tab === "student" && (
        <StudentTable data={studentData} search={search} myId={studentId} />
      )}
      {!loading && !error && tab === "outing" && (
        <OutingTable data={outingData} search={search} />
      )}
              ...
              <div className="location-grid">
                {LOCATIONS_1.map((loc) => {
                  ...
                  return (
                    <button
                      key={loc}
                      className={`loc-btn ${form.loc1 === loc ? "selected" : ""} ${full ? "full" : ""}`}
                      onClick={() => !full && setForm((f) => ({ ...f, loc1: loc }))}
                      disabled={full}
                      title={cap ? `${count}/${cap}명` : ""}
                    >
                      {loc}
                      {cap && <span className="loc-count"> {count}/{cap}</span>}
                    </button>
                  );
                })}
              </div>
              ...
              <div className="location-grid">
                {LOCATIONS_2.map((loc) => {
                  ...
                  return (
                    <button
                      key={loc}
                      className={`loc-btn ${form.loc2 === loc ? "selected" : ""} ${full ? "full" : ""}`}
                      onClick={() => !full && setForm((f) => ({ ...f, loc2: loc }))}
                      disabled={full}
                      title={cap ? `${count}/${cap}명` : ""}
                    >
                      {loc}
...
function StudentTable({ data, search, myId }) {
  ...
  return (
    <div className="table-wrap">
      <table className="data-table">
        <thead>
          <tr>
            <th>학번</th><th>이름</th><th>창의1 장소</th><th>창의1 결과</th><th>창의2 장소</th><th>창의2 결과</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
      ...
      <div className="row-count">총 {rows.length}명</div>
    </div>
  );
}
  ...
  return (
    <div className="table-wrap">
      <table className="data-table">
        <thead>
          <tr>
            <th>#</th><th>학번</th><th>이름</th><th>신청일</th><th>나가는 시각</th><th>들어오는 시각</th><th>총외출시간</th><th>교사 승인</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
